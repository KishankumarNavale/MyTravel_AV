...
//		//alert.setContentText("You are existing from Application!");
	...
	public void logoutAction() {
		...
		if(null != registerCustomerStage)
		registerCustomerStage.hide();
                Main.ls.info("User :   " +  Users.getUserName() +  "    Logout Successfull  " );
		
	}
	
	public boolean backupDataWithOutDatabase() {
		...
		Date date = new Date();
		String filepath =Util.getDbBackUpfilePath().trim()+ "AV_DB_Backup_" + Util.getDbName() + "-(" + dateFormat.format(date) + ").sql";
		String filepath1 = Util.getDbBackUpfilePath1().trim()+"AV_DB_Backup1_" + Util.getDbName() + "-(" + dateFormat.format(date) + ").sql";
		String filepath2 = Util.getDbBackUpfilePath2().trim()+"AV_DB_Backup2_" + Util.getDbName() + "-(" + dateFormat.format(date) + ").sql";
		 
		String batchCommand = "";
	
		Runtime runtime = Runtime.getRuntime();
		//p = runtime.exec("cmd /c start "+batchCommand);
		p = runtime.exec("C:/Program Files/MySQL/MySQL Server 5.5/bin/mysqldump.exe -u"+Util.getuName()+" -p"+Util.getpWord()+" -B "+Util.getDbName()+" -r"+filepath);
		
		p = runtime.exec("C:/Program Files/MySQL/MySQL Server 5.5/bin/mysqldump.exe -u"+Util.getuName()+" -p"+Util.getpWord()+" -B "+Util.getDbName()+" -r"+filepath1);
		p = runtime.exec("C:/Program Files/MySQL/MySQL Server 5.5/bin/mysqldump.exe -u"+Util.getuName()+" -p"+Util.getpWord()+" -B "+Util.getDbName()+" -r"+filepath2);
		int processComplete = p.waitFor();
		 
		if (processComplete == 0) {
		status = true;
		    System.out.println("Backup created successfully for without DB " + Util.getDbName() );
		} else {
		status = false;
		System.out.println("Could not create the backup for without DB " + Util.getDbName() );
		}
